...
//
// Готовые All и AllSettled выражают только одно правило для всей пачки. Здесь
// решение зависит от индекса конкретной задачи, поэтому нужен Stream.
...
loop:
	...
	if fatal != nil {
		...
		// оставшиеся клиентские задачи. Сервер узнаёт об отмене асинхронно,
		// поэтому строка «клиент отменил запрос» может появиться как до этой
		// строки, так и сразу после неё.
