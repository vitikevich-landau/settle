...
//
// Здесь нужен AllSettled, а не ранний выход. Оркестратору мало знать, что
// «что-то сломалось», — ему и дежурному нужен полный отчёт: кто именно лёг,
// кто ответил и за сколько. Прерваться на первой же неудаче значит потерять
// картину целиком.
//
...
func demoHealth() {
	section("Ручка /readyz: опрос всех зависимостей",
		"Полный отчёт важнее раннего выхода, поэтому используем AllSettled,\n"+
			"а рамку по времени задаёт общий дедлайн.")
	...
	start := time.Now()
	// Внутри первой завершится упавшая kafka, но AllSettled возвращает
	// результаты в порядке исходных задач. Поэтому строки отчёта не пляшут
	// от запуска к запуску и ручная раскладка по индексам не нужна.
	report := settle.AllSettled(ctx, tasks...)
...
// AllSettled ничего не занулит за вас: в Result.Value приедет ровно то, что
