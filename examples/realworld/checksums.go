...
// пачка — отдельный вызов AllSettled. Сам slices.Chunk возвращает iter.Seq,
// поэтому список при этом не копируется.
//
...
// проверять ctx между порциями данных. Без этого AllSettled честно дождётся,
// пока задача дочитает свой гигабайт до конца.
...
func demoChecksums() {
	...
	for group := range slices.Chunk(paths, batch) {
		...
		// AllSettled возвращает результаты в порядке tasks, поэтому итоговый
		// список совпадёт с исходным списком путей без ручной раскладки.
		batchResults := settle.AllSettled(ctx, tasks...)
